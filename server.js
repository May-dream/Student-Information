...
app.post('/api/submit', (req, res) => {
  const { name, studentId, major, className, phone, email } = req.body;
  const submitTime = new Date().toISOString(); // 记录提交时间

  // 插入数据库
  db.run(
    'INSERT INTO students (name, studentId, major, className, phone, email, submitTime) VALUES (?, ?, ?, ?, ?, ?, ?)',
    [name, studentId, major, className, phone, email, submitTime],
    (err) => {
      if (err) {
...
app.get('/api/students', (req, res) => {
  ...
  const conditions = [];
  if (search) {
    conditions.push(`(name LIKE ? OR studentId LIKE ? OR major LIKE ?)`);
    params = params.concat([`%${search}%`, `%${search}%`, `%${search}%`]);
  }
  if (major && major !== '') {
